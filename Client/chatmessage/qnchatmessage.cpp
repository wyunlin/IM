...
QNChatMessage::QNChatMessage(QColor color , QWidget *parent, int type) : QWidget(parent),m_type{type}
{
    ...
    m_leftPixmap = QPixmap(":/Resources/Customer Copy.png");
    m_rightPixmap = QPixmap(":/Resources/CustomerService.png");

    m_loadingMovie = new QMovie(this);
    m_loadingMovie->setFileName(":/Resources/loading4.gif");
...
void QNChatMessage::setTextSuccess(bool isSending)
{
    m_loading->hide();
    m_loadingMovie->stop();
...
void QNChatMessage::setText(bool isSending,QString text, QString time, QSize allSize, QNChatMessage::User_Type userType)
{
    m_msg = text;
    m_userType = userType;
    m_time = time;
...
void QNChatMessage::setPixmap(QPixmap pixmap)
{
    m_pixmap = pixmap;
}

QPixmap QNChatMessage::getPixmap()
{
    return m_pixmap;
}

void QNChatMessage::setImageLabel(QPixmap pixmap, QString time, QNChatMessage::User_Type userType)
{
    m_pixmap = pixmap;
    int iconWH = 40;
    int iconSpaceW = 20;
    ...
    m_iconRightRect = QRect(this->width() - iconSpaceW - iconWH, iconTMPH, iconWH, iconWH);
    m_imageLabel->show();
    m_imageLabel->setImage(m_pixmap);
    m_userType = userType;
    m_time = time;
    m_curTime = QDateTime::fromTime_t(time.toInt()).toString("hh:mm");
    m_imageY = 10;
...
void QNChatMessage::slotThemeStyle(bool isChange)
{
    if(isChange)
    {
        colme_Kuang.setRgb(255,85,0);
    }
    else
    {
        colme_Kuang.setRgb(75,164,242);
    }
    update();
}
